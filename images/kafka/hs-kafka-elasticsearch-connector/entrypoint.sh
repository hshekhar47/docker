...
echo "offset.storage.topic=${OFFSET_STORAGE_TOPIC:-'topic-connect-offset'}" >> $KAFKA_HOME/config/connect-distributed.properties
echo "config.storage.topic=${CONFIG_STORAGE_TOPIC:-'topic-connect-config'}" >> $KAFKA_HOME/config/connect-distributed.properties
echo "status.storage.topic=${STATUS_STORAGE_TOPIC:-'topic-connect-status'}" >> $KAFKA_HOME/config/connect-distributed.properties
...
if [ ! -z "$PLUGINS" ]; then
    IFS=','; for plugin_name in $PLUGINS; do
        ...
        /home/kafka/load_plugin.sh --plugin="${plugin_name}"
    done
fi
